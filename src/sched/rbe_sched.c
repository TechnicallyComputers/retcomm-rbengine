...
int rbe_sched_real_delay_enabled(void)
{
    static int s_mode = -1;
    if (s_mode < 0) {
        const char *e = getenv("RBE_RB_ZERO_DELAY");
                                    ...
                                    int count_expire)
{
    ...
    if (s_grace_ms < 0) {
        const char *e = getenv("RBE_RB_INVENT_GRACE_MS");
...
static uint32_t np_gap1_grace_cap_ms(int *has_override)
{
    ...
    if (s_cap == -2) {
        const char *e = getenv("RBE_RB_GAP1_GRACE_MS");
        if (e && e[0]) {
            s_cap = atoi(e);
            if (s_cap < 0) s_cap = 0;
...
{
    if (g_ts_enabled < 0) {
        const char *e = getenv("RBE_RB_TIMESYNC");
...
static int np_cross_os_diag_enabled(void)
{
    static int s = -1;
    if (s < 0) {
        const char *e = getenv("RBE_CROSS_OS_PACING_DIAG");
        s = (e && e[0] == '1' && e[1] == '\0') ? 1 : 0;
    }
    return s;
}
...
static void np_auto_delay_tick(uint32_t now)
{
    ...
    if (s_enabled < 0) {
        const char *e = getenv("RBE_RB_AUTO_DELAY");
                            ...
                            const char **reason_out)
{
    ...
    if (s_gap1_legacy < 0) {
        const char *e = getenv("RBE_RB_GAP1_INVENT");
